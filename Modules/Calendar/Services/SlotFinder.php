...
class SlotFinder
{
     ...
     */
    public function slots(SchedulableGoal $goal, CarbonImmutable $weekStart, array $busy): array
    {
        $slots = [];
        ...
        for ($day = 0; $day < 7; $day++) {
            ...
            foreach ($this->windows($goal, $date) as [$start, $end]) {
                $cursor = $start;

                while ($cursor->copy()->addMinutes($goal->durationMinutes)->lte($end)) {
                    $slotEnd = $cursor->addMinutes($goal->durationMinutes);
    ...
    private function windows(SchedulableGoal $goal, CarbonImmutable $date): array
    {
        $windows = $goal->preferredWindows ?: $this->defaultWindows($goal->category);
        $resolved = [];
    ...
    {
        $work = config('calendar.work_hours');
