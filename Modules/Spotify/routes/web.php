...
Route::prefix('spotify')->name('spotify.')->group(function () {
    ...
    // Shuffle & repeat
    Route::post('/shuffle', [SpotifyController::class, 'setShuffle'])->name('shuffle');
    Route::post('/repeat', [SpotifyController::class, 'setRepeatMode'])->name('repeat');

    // Devices
    Route::get('/devices', [SpotifyController::class, 'getDevices'])->name('devices');
    Route::post('/transfer-playback', [SpotifyController::class, 'transferPlayback'])->name('transfer-playback');

    // Browse
    Route::get('/recently-played', [SpotifyController::class, 'getRecentlyPlayed'])->name('recently-played');
    Route::get('/search', [SpotifyController::class, 'search'])->name('search');
    Route::get('/queue', [SpotifyController::class, 'getQueue'])->name('queue');
    Route::post('/add-to-queue', [SpotifyController::class, 'addToQueue'])->name('add-to-queue');
