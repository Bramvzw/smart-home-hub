...
<div class="spotify-next-dock {{ $hasNextTrack ? '' : 'is-empty' }}" aria-label="Volgende nummer">
    <div id="next-track" class="spotify-next-content">
        ...
        <div class="spotify-next-copy">
            <div class="spotify-next-label">Volgende</div>
            <div id="next-track-name" class="spotify-next-title">{{ $upcomingTrack?->trackName ?? 'Geen volgend nummer' }}</div>
            <div id="next-track-artists" class="spotify-next-artists">{{ $upcomingTrack?->artistNames ?? '' }}</div>
        </div>
        <button type="button" class="spotify-next-play play-next-track-btn" aria-label="Volgende afspelen" @disabled(! $hasNextTrack)>
