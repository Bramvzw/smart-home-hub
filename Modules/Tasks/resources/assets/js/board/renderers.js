...
const renderCard = (task) => {
    ...
    return `<article class="task-card ${task.completed ? 'is-completed' : ''} ${task.archived ? 'is-archived' : ''}" data-task-id="${task.id}">
        ...
        <div class="task-card-main">
            <button class="task-check" data-action="toggle-task" title="${task.completed ? 'Markeer als open' : 'Markeer als klaar'}">${task.completed ? '✓' : ''}</button>
        ...
        ${(task.due_date || checklistTotal || task.archived) ? `<div class="task-meta">
            ...
            ${task.archived ? '<span>Gearchiveerd</span>' : ''}
        </div>` : ''}
    </article>`;
};
...
const renderSidebar = (store) => `<aside class="tasks-sidebar">
    <div class="tasks-brand">
        ...
        <div>
            ...
            <span>Lokale taken</span>
        </div>
    </div>
    <div class="sidebar-heading">
        <span>Borden</span>
        <button data-action="new-board" title="Bord maken">+</button>
    </div>
...
const renderToolbar = (store) => {
    ...
    return `<header class="tasks-toolbar">
        <div class="toolbar-primary">
            <input class="board-title-input" value="${escapeHtml(store.board().name)}" data-action="rename-board" aria-label="Bordnaam">
            <span>${totalVisible}/${totalActive} zichtbaar</span>
            <button class="toolbar-icon danger" data-action="delete-board" title="Bord verwijderen">Verwijder bord</button>
            <button class="primary-action" data-action="new-task">+ Nieuwe taak</button>
        </div>
        <div class="toolbar-filters">
            <input class="search-input" placeholder="Zoeken..." value="${escapeHtml(filters.search)}" data-filter="search">
            ...
            <select data-filter="deadline">
                ...
                <option value="overdue" ${filters.deadline === 'overdue' ? 'selected' : ''}>Te laat</option>
                <option value="today" ${filters.deadline === 'today' ? 'selected' : ''}>Vandaag</option>
                <option value="week" ${filters.deadline === 'week' ? 'selected' : ''}>Deze week</option>
            </select>
            <label class="archive-toggle">
                <input type="checkbox" data-filter="showArchived" ${filters.showArchived ? 'checked' : ''}>
                Archief
            </label>
            <button class="clear-filters" data-action="clear-filters">Wis</button>
        </div>
    </header>`;
};
    ...
    ${store.board().columns.map((column) => {
        ...
        return `<section class="task-column ${column.name.toLowerCase() === 'done' ? 'done-column' : ''}" data-column-id="${column.id}">
            <header class="column-header">
                ...
                <button data-action="delete-column" title="Kolom verwijderen">×</button>
            </header>
            <div class="task-list" data-sortable-tasks data-column-id="${column.id}">
                ${visibleTasks.map(renderCard).join('')}
                ${visibleTasks.length ? '' : '<div class="column-empty">Geen taken</div>'}
            </div>
            <form class="quick-add" data-action="quick-add">
                <input name="title" placeholder="Taak toevoegen...">
                <button>+</button>
            </form>
        </section>`;
    }).join('')}
    <button class="add-column" data-action="new-column">+ Kolom</button>
</main>`;
...
const renderDetail = (store) => {
    ...
    return `<aside class="task-detail open">
        <div class="detail-head">
            <span class="priority-pill priority-${task.priority}">${task.priority === 'high' ? 'Hoog' : task.priority === 'low' ? 'Laag' : 'Normaal'}</span>
            ${task.completed ? '<span class="done-pill">Klaar</span>' : ''}
        ...
        <div class="detail-body">
            ...
            <div class="field">
                ...
                <div class="priority-segment">
                    ${['high', 'normal', 'low'].map((priority) => `<button class="${task.priority === priority ? 'active ' : ''}priority-${priority}" data-priority="${priority}">${priority === 'high' ? 'Hoog' : priority === 'low' ? 'Laag' : 'Normaal'}</button>`).join('')}
                </div>
            </div>
            <div class="field">
            ...
            <div class="field">
                ...
                <form class="new-label" data-action="new-label">
                    <input name="name" placeholder="Nieuw label">
                    <select name="color">
            ...
            <div class="field">
                ...
                <form class="new-checklist" data-action="new-checklist">
                    <input name="text" placeholder="Item toevoegen...">
                </form>
            </div>
        </div>
        <footer class="detail-foot">
            <button data-action="archive-task">${task.archived ? 'Herstellen' : 'Archiveren'}</button>
            <button class="danger" data-action="delete-task">Verwijderen</button>
            <small>${created ? `Aangemaakt ${created}` : ''}</small>
        </footer>
    </aside>`;
};
