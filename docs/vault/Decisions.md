...
| 2026-07-06 | **`config/app.php` timezone stays UTC; each module renders in local time via its own `*_TIMEZONE` key** (`WEATHER_/NEWS_/BRIEFING_/CALENDAR_TIMEZONE`, default `Europe/Amsterdam`). Calendar previously fell back to `app.timezone` and showed Google events 2h off; it now reads `calendar.timezone` everywhere (client mapping, service, viewmodel, controller, blade). | Keeping the app in UTC is safe for storage; display localization is a per-module concern already established by the other modules. | Generalization: a single hub-wide display timezone setting could replace the per-module keys. |
...
| 2026-07-08 | **News sources are generic by default, personal curation lives outside the repo.** The tracked config ships a small reachable set (Hacker News, Ars Technica, BBC World, NASA) and an empty keyword list; a deployment overrides them with `NEWS_KEYWORDS` (comma list) and `NEWS_FEEDS_FILE` (a JSON `{topics, feeds}` file, kept untracked). | The public repo should not advertise the owner's interests as the product's defaults; keeping curation in an untracked file means a pull never overwrites it and a fresh install starts neutral. | A DB-backed feed manager with UI replaces the file (the proper long-term home). |
| 2026-07-08 | **Commit convention enforced, releases tagged.** Messages follow `PREFIX: description` (taxonomy in AGENTS.md), validated by a tracked `commit-msg` hook (`core.hooksPath` set on `composer install`) and a `commits` CI job, both via `bin/lint-commit-message.sh`. `bin/release.sh` bumps the minor version, rolls `CHANGELOG.md` (Keep a Changelog), tags `vX.Y.0` and publishes a GitHub Release via `gh`. | The history was already disciplined but invisible: no tags, no releases, no changelog — the packaging is what profile visitors and employers actually see. Enforcement keeps it true without relying on memory. | Multi-contributor workflow arrives → consider PR-based flow and protected main. |
| 2026-07-08 | **Modules can be disabled and reordered from /settings** via `ModuleState` (SettingsStore keys `modules.{slug}.enabled` / `modules.{slug}.order`). `bootstrap/providers.php` stays the single source of truth for what is *loaded*: a disabled module keeps its routes, container bindings, contracts (`SchedulableGoals`) and briefing sources registered — `ModuleRegistry` only filters and sorts everything user-facing (navigation, dashboard, settings panes), and its scheduled jobs are skipped through `->when(...)` gates evaluated at run time. | Unloading providers would amount to a second module status registry (forbidden by AGENTS.md) and would break cross-module seams; visibility-state makes disabling instant, safe and reversible without a redeploy. | A module ever needs hard unloading (e.g. a security concern) → revisit provider-level toggling. |
| 2026-07-06 | **Calendar reads every calendar the user has selected in Google, not just `primary`.** `GoogleCalendarClient` fetches `users/me/calendarList`, keeps the `selected` calendars, and pulls events per calendar carrying that calendar's own label + `backgroundColor`; `busyTimes()` feeds all of them to `freeBusy` so the planner respects non-primary commitments. Falls back to the single configured `calendar_id` when the list call fails. `GOOGLE_CALENDAR_ID` now only scopes writes (planner inserts) and the fallback. | The owner's real agenda lives across 6 calendars (family/shared/holidays); reading only `primary` surfaced ~2 of ~47 events. | A per-calendar visibility toggle in settings is wanted → filter the selected set by config instead of Google's `selected` flag. |
