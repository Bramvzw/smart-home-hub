...
ssh -i "$NAS_KEY" "$NAS_SSH" \
  "sudo $DC exec -T hub sh -c 'cd /app && find .git -name @eaDir -prune -exec rm -rf {} + 2>/dev/null; git pull --ff-only origin main && php artisan migrate --force && php artisan optimize'"
